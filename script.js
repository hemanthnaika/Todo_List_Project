...
const createListItem = (chore, id) => {
    ...
    <i class="fas fa-times cross-${id}">X</i>  
`
}
...
const addChore = (input) => {
    ...
    chore.length === 0 ? alert("Enter the task") : chores.find(item => item.name == chore) ? alert(`${chore}` + " Item already exists") : handleValidChore(chore)
    select('.taskbar div:nth-child(1)').innerHTML = `${countItems(chores, filter)} items`
